program sp3mm_test_exact_indices
    use omp_lib
    use psb_base_mod
    use sp3mm_test_mod
    implicit none
    ...
    call spmm_idx_map_row_by_row(r, ac, rac, info)
    spmm1_time = omp_get_wtime() - spmm1_time
    if (info /= 0) error stop
    spmm2_time = omp_get_wtime()
    call spmm_idx_map_row_by_row(rac, p, out_to_check,info)
...
end program sp3mm_test_exact_indices