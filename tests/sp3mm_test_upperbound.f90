program sp3mm_test_upperbound
    ...
    tic = omp_get_wtime()
    call spmm_upper_bound_row_by_row(r, ac, rac, info)
    if (info /= 0) return
    call spmm_upper_bound_row_by_row(rac, p, out_to_check,info)
    if (info /= 0) return
    toc = omp_get_wtime()
...
end program sp3mm_test_upperbound