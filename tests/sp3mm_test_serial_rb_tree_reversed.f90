program sp3mm_test_serial_rb_tree_reversed
    use omp_lib
    use psb_base_mod
    use sp3mm_test_mod
    implicit none
    ...
    if (info /= 0) then
        ...
        call spmm_serial_rb_tree(ac, p, acp, info)
        spmm1_time = omp_get_wtime() - spmm1_time
        if (info /= 0) error stop
        spmm2_time = omp_get_wtime()
        call spmm_serial_rb_tree(r, acp, out_to_check,info)
        ...
        if (command_argument_count() >= 5) then
            ...
            write (*, '(A,",",A,",",A,",",A,",",A,",",ES12.6,",",I0,",",I0,",", &
                        I0,",",I0,",",ES12.6,",",I0,",",I0,",",I0,",",ES12.6)') &
                                        "serial_rb_tree_reversed", trim(collection), &
...
end program sp3mm_test_serial_rb_tree_reversed