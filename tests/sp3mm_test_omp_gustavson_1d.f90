program sp3mm_test_omp_gustavson_1d
    use omp_lib
    use psb_base_mod
    use sp3mm_test_mod
    implicit none
    ...
    integer(psb_ipk_) :: info, i, iter, iterations
    ...
    call get_environment_variable("SP3MM_ITERATIONS", iterations_str, status=info)
    if (info /= 0) then
        iterations = 1
    else
        read (iterations_str, *) iterations
    end if

    do iter = 1, iterations
        spmm1_time = omp_get_wtime()
        call spmm_omp_gustavson_1d(r, ac, rac, info)
        spmm1_time = omp_get_wtime() - spmm1_time
        if (info /= 0) error stop
        spmm2_time = omp_get_wtime()
        call spmm_omp_gustavson_1d(rac, p, out_to_check,info)
        ...
        if (command_argument_count() >= 5) then
            ...
            write (*, '(A,",",A,",",A,",",A,",",A,",",ES12.6,",",I0,",",I0,",", &
                        I0,",",I0,",",ES12.6,",",I0,",",I0,",",I0,",",ES12.6)') &
                                        "gustavson_1d", trim(collection), &
                                        trim(size), trim(smoothing),&
                                        trim(level), preparing_time,&
                                        omp_get_max_threads(),&
                                        rac%get_nrows(), rac%get_ncols(),&
                                        rac%get_nzeros(), spmm1_time,&
            ...
            write (*,*)
            write (*, '("R * AC: ", ES12.6)') spmm1_time

            write (*,*)
            write (*, '("RAC * P: ", ES12.6)') spmm2_time
        end if
    end do
end program sp3mm_test_omp_gustavson_1d