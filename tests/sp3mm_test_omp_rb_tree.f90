program sp3mm_omp_rb_tree
    ...
    if (info /= 0) then
        ...
        call spmm_omp_rb_tree(r, ac, rac, info)
        spmm1_time = omp_get_wtime() - spmm1_time
        if (info /= 0) error stop
        spmm2_time = omp_get_wtime()
        call spmm_omp_rb_tree(rac, p, out_to_check,info)
