program sp3mm_test_omp_two_pass
    use omp_lib
    use psb_base_mod
    use sp3mm_test_mod
    implicit none
    ...
    if (info /= 0) then
        ...
        call spmm_omp_two_pass(r, ac, rac, info)
        spmm1_time = omp_get_wtime() - spmm1_time
        if (info /= 0) error stop
        spmm2_time = omp_get_wtime()
        call spmm_omp_two_pass(rac, p, out_to_check,info)
...
end program sp3mm_test_omp_two_pass